...
def HRRN(process_list: list):
    ...
    while len(act) > 0:
        ...
        for i in range(0, len(act)):
            if act[i].arrival_time < current_time:
                temp = (act[i].burst_time + (current_time - act[i].arrival_time)) /\
                    act[i].burst_time
    ...
    for i in range(0, len(process_list)):
        print(process_list[i].process_name, " arrival time :",
              process_list[i].arrival_time, " burst time :",
              process_list[i].burst_time, " waiting time :",
              process_list[i].waiting_time, " turn around time :",
